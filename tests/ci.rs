...
fn version_prerelease_is_date_of_version_bump_or_latest_tag() {
    ...
    let version_bump_date = if latest_commit_subject == VERSION_BUMP_SUBJECT {
        rev_date(&latest_commit).map(Some).unwrap()
    } else {
        None
    };
    ...
    if enabled("BLESS") {
        ...
        amend_latest_commit().unwrap();
        let amended_date = rev_date("HEAD").unwrap();
...
fn rev_date(rev: &str) -> Result<String> {
    let mut command = Command::new("git");
...
fn amend_latest_commit() -> Result<()> {
    let mut command = Command::new("git");
    command.args(["commit", "--amend", "--no-edit"]);
    let status = command.status()?;
